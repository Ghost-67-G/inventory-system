...
export function InviteUserModal({ open, onClose }: InviteUserModalProps) {
  ...
  // Reset form + error state on every close (Cancel / X / Escape / overlay)
  // so a cancelled invite doesn't reappear the next time the modal is opened.
  const handleClose = () => {
    setError(null);
    reset({ role: 'staff' });
    onClose();
    ...
    >
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Invite a teammate to your workspace.</DialogDescription>
        </DialogHeader>
